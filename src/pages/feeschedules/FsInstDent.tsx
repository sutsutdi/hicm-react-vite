...
export default function FsInstDentPage() {
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'age', headerName: 'อายุ', width: 150 },
    ...
    { field: 'icd10', headerName: 'ICD10', width: 280 },
    { field: 'codeinst', headerName: 'code inst', width: 280 },
    { field: 'nameprscdt', headerName: 'name prsdt', width: 280 },
    { field: 'procedure_opd', headerName: 'ICD9CM', width: 280 },
  ...
  const onSubmit = async () => {
    ...
    try {
      const response = await axios.post(`${apiHiUrl}/fs/instdent`, {
        startDate,
        endDate,
      })
  ...
  const titleClaim = 'Instrument Dental'
        ...
        {isLoading ? (
          ...
          <Card sx={{ width: 645, marginLeft: '50px' }}>
            <Stack direction={'row'} gap={2} padding={'10px'}>
              <Typography variant="h6">Case {titleClaim}</Typography>
            </Stack>
            <Divider />
            <Stack direction={'row'} gap={2} padding={'10px'}>
      ...
      <Box sx={{ width: '100%', height: '500px', typography: 'body1' }}>
        <TabContext value={valueTab}>
          <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
            ...
            >
              <Tab label={`Case ${titleClaim}`} value="1" />

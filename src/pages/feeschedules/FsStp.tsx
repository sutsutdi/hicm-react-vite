...
export default function FsUcepPage() {
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'ovstosะ', headerName: 'ovstost', width: 280 },
    { field: 'nameovstos', headerName: 'สถานะจำหน่าย', width: 280 },
  ]
  ...
  const onSubmit = async () => {
    ...
    try {
      const response = await axios.post(`${apiHiUrl}/fs/stp`, {
        startDate,
        endDate,
      })
        ...
        {isLoading ? (
          ...
          <Card sx={{ width: 645, marginLeft: '50px' }}>
            <Stack direction={'row'} gap={2} padding={'10px'}>
              <Typography variant="h6">Case ER UCEP</Typography>
            </Stack>
            <Divider />
            <Stack direction={'row'} gap={2} padding={'10px'}>
